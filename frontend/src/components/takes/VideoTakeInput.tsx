...
export function VideoTakeInput({ sceneId }: { sceneId: string }) {
  ...
  const processTake = async () => {
    ...
    const response = await fetch(`${api}/api/scenes/${encodeURIComponent(sceneId)}/demo-takes`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ take_number: nextTake, character: "MARCUS", line: transcript.trim(), file_name: file.name }),
    });
